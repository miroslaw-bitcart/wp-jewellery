...
foreach ($items as $item) :
	...
	$image = ( $show_image ) ? '<img src="' . current( $attachment_image_src ) . '" alt="Product Image" height="' . $image_size[1] . '" width="' . $image_size[0] . '" style="vertical-align:middle; margin-right: 12px;" />' : '';

	?>
	<tr>
		<td style="text-align:left;vertical-align:middle;border: 1px solid #eee;word-wrap:break-word;font-family: georgia,serif;"><?php
		...
		?></td>
		<td style="text-align:left;vertical-align:middle;border: 1px solid #eee;font-weight:normal;font-family:georgia,serif;"><?php echo $order->get_formatted_line_subtotal( $item ); ?></td>
	</tr>

	<?php if ($show_purchase_note && $purchase_note = get_post_meta( $_product->id, '_purchase_note', true)) : ?>
		<tr>
			<td colspan="2" style="text-align:left;vertical-align:middle;border: 1px solid #eee;font-family:georgia,serif;"><?php echo apply_filters('the_content', $purchase_note); ?></td>
		</tr>
	<?php endif; ?>
