...
<?php if ( ( $woocommerce->cart->needs_shipping() || get_option('woocommerce_require_shipping_address') == 'yes' ) && ! $woocommerce->cart->ship_to_billing_address_only() ) : ?>
	...
	<h3 class="border-bottom deco"><?php _e( 'Delivery Address', 'woocommerce' ); ?></h3>

	<p style="margin-bottom:30px;" id="shiptobilling">
		<label for="shiptobilling-checkbox" class="checkbox"><?php _e( 'Deliver to billing address?', 'woocommerce' ); ?></label>
