...
jQuery( document ).ready( function( $ ) {
	...
	$( "#price-search-slider" ).slider({
		...
		slide: function( event, ui ) {
			$( "#price-low" ).text( 'Â£' + ui.values[ 0 ] );
			$( "#price-high" ).text( 'Â£' + ui.values[ 1 ] );
		},
		values: [ 0, 5000 ],
...
var DynamicView = function() {
	...
	var getQueryString = function() {
                ...
                var reParam = reParamUrl(current_Url);
        ...
        var reParamUrl = function(cut_url) {
            
            var n = cut_url.search("&post_type=product");
            var res;
            
            if(n == -1)
                res = '';
            else {
                var m = cut_url.indexOf("?s=");
                
                if(m == -1) {
                    var j = cut_url.indexOf("&s=");
                    
                    j += 1;
                    
                    res = cut_url.substr(j);
                    
                } else {
                    
                    m += 1;
                    res = cut_url.substr(m);
                }              
	...
	var getQueryAttributes = function() {
		...
		return {
			...
			ollys_picks: (function() {
				var ex = new RegExp('/ollys-picks/([^/]+)/?'); // hubris
				var loc = window.location.toString();
				var matches = loc.match(ex);
				if( matches && matches[1] )
					return matches[1]; 
			...
			period: (function() {
				var ex = new RegExp('/period/([^/]+)/?'); // hubris
				var loc = window.location.toString();
				var matches = loc.match(ex);
				if( matches && matches[1] )
					return matches[1]; 
	...
	this.getProducts = function() {
		...
		// console.log(getQueryAttributes());
...
$( "#banner-search" ).click(function() {
	$( ".search-toggle" ).fadeToggle(200);
	$( "#banner-search" ).fadeOut(200);
});	

$( "#close-search" ).click(function() {
	$( ".search-toggle" ).fadeOut(200);
	$( "#banner-search" ).fadeToggle(200);
	return false;
});	
...
$ = jQuery;
$(function () {
	jQuery('body').on('click', 'a.quick-view', function(e) {
		e.preventDefault();

		var modal = $('#product');
		var modalBody = $('#product .modal-content');
		var pid = pid1 = $(this).data('id');
		var loaded_modal = 0;
		// console.log('pid', pid, pid1);
		modal.on('show.bs.modal', function () {
			if(loaded_modal) return;
			if(pid != pid1) return;
			// console.log('show.bs.modal', pid, pid1);
			$.ajax({
				type: "POST",
				url: _Archetype.ajaxurl,
				data: {
					'action': 'get_product_modal_html',
					'pid': pid,
				},
				dataType: 'html',
