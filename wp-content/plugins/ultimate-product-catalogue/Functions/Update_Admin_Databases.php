...
//$wpdb->show_errors();
    ...
    $objReader = PHPExcel_IOFactory::createReader($inputFileType);
		...
		foreach ($Titles as $key => $Title) {
				if ($Title != "" and !array_key_exists($Title, $Allowed_Fields) and !array_key_exists($Title, $Allowable_Custom_Fields) and strpos($Title, "Additional Image") === FALSE and strpos($Title, "Video ID") === FALSE) {
		...
		for ($column = 0; $column < $highestColumnIndex; $column++) {
				if ($Allowed_Fields[$Titles[$column]] != "Tags_Names_String" and $Allowed_Fields[$Titles[$column]] != "Catalogue_ID" and !array_key_exists($column, $Custom_Fields) and !in_array($column, $Additional_Images_Cols) and !in_array($column, $Video_IDs_Cols)) {$Fields[] = $Allowed_Fields[$Titles[$column]];}
		...
		foreach ($Data as $Product) {
				...
				foreach ($Product as $Col_Index => $Value) {
						if ((!isset($Tags_Column) or $Tags_Column != $Col_Index) and (!isset($Cat_ID_Column) or $Cat_ID_Column != $Col_Index) and !array_key_exists($Col_Index, $Custom_Fields) and !in_array($Col_Index, $Additional_Images_Cols) and !in_array($Col_Index, $Video_IDs_Cols)) {$Values[] = esc_sql($Value);}
...
function Update_UPCP_Options() {
	...
	if (!isset($_POST['woocommerce_sync'])){ $_POST['woocommerce_sync']	= "";}
	if (!isset($_POST['extra_elements'])){ $_POST['extra_elements'] = "";}
