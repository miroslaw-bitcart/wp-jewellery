		...
		foreach ($Gridster as $Element) {
			switch ($Element->element_class) {
				...
				case "custom_field":
					...
					else {if(is_object($Field_Value)){$ProductString .= $Field_Value->Meta_Value;}}
					break;
				case "custom_label":
	...
	if (is_array($Fields)) {
		...
		foreach ($Fields as $Field) {
			...
			if ($Meta_Value != "" or $Custom_Fields_Blank != "Yes") {
				...
				else {$CustomFieldString .= $AddBreak . "<span class='upcp-cf-label'>" . $Field->Field_Name . ":</span><span class='upcp-cf-value'>" . $Meta_Value . "</span>";}
				$AddBreak = "<br />";
			}
		}
