...
class WC_Product_Factory {
	...
	public function get_product( $the_product = false, $args = array() ) {
		...
		if ( in_array( $post_type, array( 'product', 'product_variation' ) ) ) {
			...
			//$classname = 'WC_Product_' . preg_replace_callback( '/-(.)/e', "'_' . strtoupper( '$1' )", ucfirst( $product_type ) );                        
                        $classname = 'WC_Product_' . preg_replace_callback( '/-(.)/', function() { return "'_' . strtoupper( '$1' )";}, ucfirst( $product_type ) );
