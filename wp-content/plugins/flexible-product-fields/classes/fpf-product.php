...
class FPF_Product {
    ...
    public function __construct(WPDesk_Plugin_1_2 $plugin, FPF_Product_Fields $product_fields ) {
        $this->_plugin = $plugin;
    ...
    public function hooks() {
        ...
        add_filter( 'woocommerce_add_to_cart_validation', array( $this, 'woocommerce_add_to_cart_validation' ), 999, 3 );
    }
    ...
    private function validate_field( $field, $value ) {
        if ( isset( $field['required'] ) && $field['required'] == '1' && ( empty( $value ) || $value == '' ) ) {
            throw new Exception( sprintf( __( '<strong>%s</strong>  is required field.', 'flexible-product-fields' ), wpdesk__( $field['title'], 'flexible-product-fields' ) ) );
        }
    }
    ...
    public function woocommerce_add_to_cart_validation( $passed, $product_id, $qty, $post_data = null ) {
        ...
        $fields = $this->get_fields_for_product( $product );
    ...
    public function add_to_cart_url( $url ) {
        global $product;
        if ( $product instanceof WC_Product && !is_single() ) {
            if ( $this->product_has_required_field( $product ) ) {
    ...
    public function create_fields_for_product( $product, $hook ) {
        $fields = $this->get_fields_for_product( $product, $hook );
        foreach ( $fields['fields'] as $field ) {
    ...
    public function create_field( array $field, WC_Product $product ) {
        $args = array(
            'type'          => $field['type'],
            'label'         => wpdesk__( $field['title'], 'flexible-product-fields' ),
            'placeholder'   => isset( $field['placeholder'] ) && $field['placeholder'] != '' ? wpdesk__( $field['placeholder'], 'flexible-product-fields' ) : '',
        ...
        if ( $field_type['has_price'] ) {
            ...
            if ( isset( $field['price_type'] ) && $field['price_type'] != '' && isset( $field['price'] ) && $field['price'] != '' ) {
                $price = $this->_product_fields->calculate_price( floatval( $field['price'] ), $field['price_type'], $product );
        ...
        if ( $field_type['has_options'] ) {
            ...
            if ( is_array( $field['options'] ) ) {
                foreach ( $field['options'] as $option ) {
                    $options[$option['value']] = wpdesk__( $option['label'], 'flexible-product-fields' );
                    if ( isset( $option['price_type'] ) && $option['price_type'] != '' && isset( $option['price'] ) && $option['price'] != '' ) {
                        $price = $this->_product_fields->calculate_price( floatval( $option['price'] ), $option['price_type'], $product );
                        $price = $this->wc_price( $price );
        ...
        if ( $field['type'] == 'heading' ) {
            $ret = $this->_plugin->load_template( 'heading', 'fields', array( 'title' => wpdesk__( $field['title'], 'flexible-product-fields' ), 'type' => $field['type'] ) );
        }
    ...
    public function woocommerce_before_add_to_cart_button() {
        ...
        foreach ( $fields['fields'] as $key => $field ) {
            ...
            if ( $field['has_price'] && isset($field['price_type']) && $field['price_type'] != '' && isset($field['price']) && $field['price'] != '' ) {
                $fields['fields'][$key]['price_value'] = $this->_product_fields->calculate_price( floatval($field['price']), $field['price_type'], $product );
                $fields['fields'][$key]['price_display'] = $this->wc_price( $fields['fields'][$key]['price_value'] );
            }
            if ( $field['has_options'] ) {
                foreach ( $fields['fields'][$key]['options'] as $option_key => $option ) {
                    $fields['fields'][$key]['options'][$option_key]['price_value'] = 0;
                    if ( isset($option['price_type']) && $option['price_type'] != '' && isset($option['price']) && $option['price'] != '' ) {
                        $fields['fields'][$key]['options'][$option_key]['price_value'] = $this->_product_fields->calculate_price( floatval($option['price']), $option['price_type'], $product );
                        $fields['fields'][$key]['options'][$option_key]['price_display'] = $this->wc_price( $fields['fields'][$key]['options'][$option_key]['price_value'] );
                    }
                }
            }
        }
